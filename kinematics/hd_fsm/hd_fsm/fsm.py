...
def str_pad(x, length=5):  # TODO move to utils
    s = str(x)
    if len(s) >= length:
        return s[:length]
...
def str_pad_list(l, length=5):  # TODO move to utils
    pad_fct = lambda x: str_pad(x, length)
    return "[ " + ", ".join(map(pad_fct, l)) + " ]"


def normalize(l):  # TODO use numpy instead
...
class FSM(Node):
    # TODO create an enum or a class for modes general to the whole system
    IDLE = -1
    MANUAL_INVERSE = 0
    MANUAL_DIRECT = 1
    SEMI_AUTONOMOUS = 2
    ...
    def __init__(self):
        ...
        self.velocity = 0
        self.command_expiration = 0.5  # seconds
    ...
    def create_ros_interfaces(self):
        self.manual_direct_cmd_pub = self.create_publisher(
            Float64MultiArray, "/HD/fsm/joint_vel_cmd", 10
        )
        self.manual_inverse_cmd_pub = self.create_publisher(
            Float64MultiArray, "/HD/fsm/man_inv_axis_cmd", 10
        )
        self.manual_inverse_twist_pub = self.create_publisher(
            TwistStamped, "/HD/fsm/man_inv_twist", 10
        )
        ...
        self.create_subscription(
            Float32MultiArray, "/CS/HD_gamepad", self.manual_cmd_callback, 10
        )
        self.create_subscription(
            Float32MultiArray, "/ROVER/HD_man_inv_axis", self.manual_cmd_callback, 10
        )
        self.create_subscription(
            Twist, "/ROVER/HD_man_inv_twist", self.manual_cmd_callback, 10
        )
        self.create_subscription(
            Float64MultiArray, "/ROVER/HD_man_inv_joint", self.manual_cmd_callback, 10
        )
        self.create_subscription(Int8, "/ROVER/HD_mode", self.mode_callback, 10)
        self.create_subscription(
            Task, "/ROVER/semi_auto_task", self.task_cmd_callback, 10
        )
        self.create_subscription(
            Int8, "/ROVER/HD_element_id", self.task_cmd_callback2, 10
        )
    ...
    def task_cmd_callback2(self, msg: Int8):  # TODO
        ...
        self.get_logger().info(
            "AAAAAAAAAAAAAAAAAAA :   " + str(x) + "  ;  " + str(task_type)
        )  # TODO make log more explicit what is AAAAAAAAA ?
        self.semi_autonomous_command = Task(type=task_type, str_slot=task_str)

    def send_task_cmd(self):  # TODO what is this function ?
    ...
    def send_manual_inverse_cmd(self):
        ...
        msg.data = array.array(
            "d", self.manual_inverse_axis + [self.manual_inverse_velocity_scaling]
        )
        if VERBOSE:
            self.get_logger().info(
                "FSM manual inverse cmd :   " + str_pad_list(list(msg.data))
            )
    ...
    def manual_direct_command_old(self):
        return (
            time.time() - self.received_manual_direct_cmd_at > self.command_expiration
        )

    def manual_inverse_command_old(self):
        return (
            time.time() - self.received_manual_inverse_cmd_at > self.command_expiration
        )
