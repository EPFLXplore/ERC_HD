...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("moveit_api_test2");
...
int main(int argc, char** argv)
{
  ...
  auto node = rclcpp::Node::make_shared("moveit_api_test2", node_options);
  ...
  collision_request.group_name = "astra_arm_group";
  current_state.setToRandomPositions();
  collision_result.clear();
  planning_scene.checkSelfCollision(collision_request, collision_result);
  ...
  std::vector<double> joint_values = { 0.0, 0.0, 0.0, -2.9, 0.0, 1.4};
  const moveit::core::JointModelGroup* joint_model_group = current_state.getJointModelGroup("astra_arm_group");
  ...
  desired_pose.header.frame_id = "gripper_base";
  ...
  bool state_valid = planning_scene.isStateValid(copied_state, kinematic_constraint_set, "astra_arm_group");
