  ...
  astra_interfaces__msg__CmdOutcome__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    astra_interfaces__msg__CmdOutcome * data =
      (astra_interfaces__msg__CmdOutcome *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!astra_interfaces__msg__CmdOutcome__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          astra_interfaces__msg__CmdOutcome__fini(&output->data[i]);
        }
        return false;
      }
    }
