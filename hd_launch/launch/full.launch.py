...
def declare_binary_launch_argument(name, default=True, description=""):
    if not isinstance(default, bool):
        raise ValueError(
            f"Default value of binary launch argument should be of type bool not {type(default)}"
        )
...
def generate_launch_description():
    sim_arg, sim_declaration = declare_binary_launch_argument(
        "sim", default=False, description="Run in simulation mode"
    )
    rviz_arg, rviz_declaration = declare_binary_launch_argument(
        "rviz", default=True, description="Run RViz"
    )
    fake_cs_arg, fake_cs_declaration = declare_binary_launch_argument(
        "fake_cs", default=True, description="Run fake control station"
    )
    camera_arg, camera_declaration = declare_binary_launch_argument(
        "vision", default=True, description="Run camera node for vision"
    )
    ...
    fsm_node = Node(package="hd_fsm", executable="fsm")

    fake_cs_node = Node(
        package="fake_components",
        executable="fake_cs_gamepad.py",
        condition=IfCondition(
            PythonExpression([fake_cs_arg, "== True"])
        ),  # Run if the fake CS is needed
    )
    ...
    fake_motor_control_node = Node(
        ...
        condition=IfCondition(
            PythonExpression([sim_arg, "== True"])
        ),  # Run if we are in simulation
    )
    ...
    real_motor_control_node = Node(  # TODO: edit motor_control node in order to be runnable like that (without the path to the cofig)
        ...
        condition=IfCondition(
            PythonExpression([sim_arg, "== False"])
        ),  # Do not run if we are in simulation
    )
    ...
    vision_node = Node(
        ...
        condition=IfCondition(
            PythonExpression([camera_arg, "== True"])
        ),  # Run if the fake CS is needed
    )
