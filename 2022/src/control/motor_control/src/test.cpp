...
double target_torque[MAX_MOTOR_COUNT] = {0};
//double gripper_target_torque = 0;
...
static const double min_qc[MAX_MOTOR_COUNT] = {-(2<<18), -INF, -INF, -((2<<18)/16), -160000, -((2<<13)/2), -10000000000};
static const double max_qc[MAX_MOTOR_COUNT] = {2<<18, INF, INF, (2<<18)/16, 160000, (2<<13)/2, 10000000000};
...
static const double reduction[MAX_MOTOR_COUNT] = {1000, 20000, 5, 50, 50*1.5, 50*1.5, 10000, 100};
...
void manualCommandCallback(const std_msgs::Float32MultiArray::ConstPtr& msg) {
    ...
    for (size_t it=0; it<MOTOR_COUNT; ++it) {
		...
		target_torque[it] = double(max_torque*vel);
        cout << setw(9) << target_torque[it];
    }
    cout << endl;
        ...
        if (chain[it]->get_has_motor()) {
            ...
            if (chain[it]->get_Device_State_In_String() == "Operation enable") {
                switch (control_mode) {
                    case Epos4::position_CSP:
                        if (!stopped) {
							std::cout << "SETTING       " << target_pos[it] << std::endl;
                            if (it == 1) {
					...
					case Epos4::torque_CST:
						chain[it]->set_Target_Torque_In_Nm(target_torque[it]);
...
int main(int argc, char **argv) {
	...
	ros::Subscriber pos_sub = n.subscribe<std_msgs::Int32>("/aaaa", 10, posCallback);
    ...
    while (ros::ok()){
		...
		if (is_scanning) {
			chain[1]->set_Target_Position_In_Qc(chain[1]->get_Actual_Position_In_Qc());
		}
